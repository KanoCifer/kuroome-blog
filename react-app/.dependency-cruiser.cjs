...
// lives in SUBFOLDERS (api/, components/, hooks/, stores/, lib/, ...) and
// is private. Packages reach each other only through entry points.
//
// Freely importable (NOT packages): assets/, test/, constants/, types/, data/.
// packages/ is documentation only.
...
/** A package's __tests__/ or tests/ folder ($1 = full package path). */
const PACKAGE_TESTS = `^${R}/(${PKG})\\/(__tests__|tests)\\/`;
...
module.exports = {
  forbidden: [
    ...
    },
    {
      ...
      to: {
        path: PACKAGE_INTERNALS,
        pathNot: `^${R}/$1/(__tests__|tests)/`, // own __tests__/tests fixtures → allowed
      },
    },
    {
      name: 'tests-folder-is-private',
      comment:
        "A package's __tests__/tests/ folder is reachable only from tests — nothing else may import fixtures.",
      severity: 'error',

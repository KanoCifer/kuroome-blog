<template>
  <div class="bg-background flex min-h-screen w-full flex-col">
    ...
    >
      <div>
        <h1 class="text-foreground font-serif text-2xl leading-tight font-medium tracking-tight">
          开发任务
        </h1>
        <p class="text-muted-foreground mt-0.5 text-sm">网站开发需求与实现清单</p>
      </div>
      ...
      <template v-else>
        ...
        <div v-show="activeTab === 'frontier'" class="space-y-8">
          ...
          <section>
            <div class="mb-3 flex items-baseline justify-between">
              <h2 class="text-foreground font-serif text-lg font-medium tracking-tight">
                现在能做什么
              </h2>
        ...
        <div v-show="activeTab === 'planning'" class="space-y-4">
          ...
          <div class="border-border overflow-hidden rounded-xl border">
            <div
              class="text-muted-foreground grid grid-cols-[2fr_1fr_1fr_1fr_100px_32px] gap-4 border-b bg-muted px-4 py-2.5 text-[10px] font-medium tracking-widest uppercase max-sm:grid-cols-[1fr_80px_32px]"
            >
              <span>标题</span>
            ...
            <div
              ...
              :key="task.id"
              class="grid cursor-pointer grid-cols-[2fr_1fr_1fr_1fr_100px_32px] items-center gap-4 border-t px-4 py-2.5 transition-colors hover:bg-muted/40 max-sm:grid-cols-[1fr_80px_32px]"
              role="button"
              tabindex="0"
        ...
        <div v-show="activeTab === 'review'" class="space-y-8">
          ...
          <section>
            ...
            <div class="space-y-3">
              ...
              >
                <span class="text-foreground w-16 shrink-0 text-sm">{{ row.type }}</span>
          ...
          <section>
            ...
            <div class="space-y-0">
              ...
              >
                ...
                />
                <div class="min-w-0 flex-1 cursor-pointer" @click="openDetail(task.id)">
                  <p
...
async function handleUpdate(id: string, patch: Parameters<typeof store.updateTask>[1]) {
...
function toggleFilter(key: 'type' | 'priority', val: DevTaskType | DevTaskPriority) {
  if (key === 'type') {
...
const filteredPlanning = computed(() =>
  store.tasks
    ...
    .filter((t) => (filterType.value.size ? filterType.value.has(t.type) : true))
    .filter((t) =>

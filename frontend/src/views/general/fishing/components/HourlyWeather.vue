...
const chartOption = computed(() => {
  ...
  return {
    ...
    yAxis: [
      ...
      {
        type: "value",
        max: (value: { max: number }) => Math.ceil((value.max * 1.3) / 10) * 10,
        min: 0,
        position: "right",
        nameTextStyle: { color: subTextColor, fontSize: 11 },
        axisLine: { show: false },
        axisTick: { show: false },
        splitLine: { lineStyle: { color: gridLineColor, type: "dashed", width: 1 } },
    ...
    series: [
      ...
      },
      {
        ...
        markArea: {
          silent: true,
          data: [[{ yAxis: 20, itemStyle: { color: "rgba(255, 188, 120, 0.1)" } }, { yAxis: 28 }]],
        },
      },
    ],
  };

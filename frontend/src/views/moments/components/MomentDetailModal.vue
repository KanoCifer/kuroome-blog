<template>
  <AlertDialog :open="open" @update:open="(v) => emit('update:open', v)">
    <AlertDialogContent
      class="bg-card border-border/40 max-h-[88vh] w-full gap-0 overflow-hidden p-0 sm:max-w-[720px]"
    >
      <!-- 顶栏：卷序 + 关闭 + admin 操作 -->
      <header
      ...
      <!-- 主体 -->
      <div v-if="moment" class="grid grid-cols-1 gap-0 md:grid-cols-[minmax(0,1fr)_220px]">
        <!-- 左：内容 -->
        <div class="overflow-y-auto px-8 py-8" style="max-height: calc(88vh - 64px)">
          <!-- 心情 -->
          <div
            v-if="moment.mood"
        ...
        >
          <div class="text-muted-foreground sticky top-4 font-mono text-[10px] tracking-[0.18em] uppercase">
            META
          </div>
          ...
          <div class="text-muted-foreground mt-6 border-t border-dashed border-border/40 pt-4 font-mono text-[10px] tracking-[0.18em] uppercase">
            KEYBOARD
          </div>

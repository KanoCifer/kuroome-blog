...
const lastRecord = ref("2天前");
const tideStatus = ref("涨潮中");
const weatherCode = ref(0);

const weatherStatus = computed(() => {
  const map: Record<number, { icon: Component; text: string }> = {
    0: { icon: Sun, text: "晴" },
    1: { icon: Cloud, text: "多云" },
    2: { icon: CloudRain, text: "小雨" },
    3: { icon: Wind, text: "大风" },
  };
  return map[weatherCode.value] || { icon: Sun, text: "晴" };
});

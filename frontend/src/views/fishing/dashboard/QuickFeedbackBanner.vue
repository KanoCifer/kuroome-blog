...
<template>
  ...
  >
    ...
    >
      <div>
        <p class="text-foreground font-family-averia text-base italic">
          钓完了？告诉我们今天实际如何
        </p>
      ...
      <button
        v-if="!disabled"
        class="bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors"
        type="button"
        @click="$emit('submit')"
      >
        反馈今日指数

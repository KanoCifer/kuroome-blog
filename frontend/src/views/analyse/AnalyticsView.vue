<template>
  <BasicDetail title="Analytics Dashboard" subtitle="Admin Only · Monitor your website performance">
    <!-- Error Message - Full Width -->
    <div v-if="error" class="col-span-1 sm:col-span-2 lg:col-span-3">
      <div class="border-destructive/30 bg-destructive/10 text-destructive squircle border p-4">
    ...
    <div class="col-span-1 sm:col-span-2 lg:col-span-3">
      ...
      <template v-if="loading && !overviewData">
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <div v-for="i in 4" :key="i" class="bg-card/60 squircle h-32 animate-pulse"></div>
        </div>
      </template>
      ...
      >
        <div class="mb-3 flex items-center gap-2">
          <div class="bg-primary/15 text-primary flex h-10 w-10 items-center justify-center rounded-xl">
          ...
          <span class="text-muted-foreground text-sm font-medium">Total Visits</span>
        </div>
        <p class="text-foreground text-3xl font-bold">{{ formatNumber(overviewData?.total_visits ?? 0) }}</p>
      ...
      >
        <div class="mb-3 flex items-center gap-2">
          <div class="bg-success/20 text-success flex h-10 w-10 items-center justify-center rounded-xl">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <span class="text-muted-foreground text-sm font-medium">Unique Visitors</span>
        </div>
        <p class="text-foreground text-3xl font-bold">{{ formatNumber(overviewData?.unique_visitors ?? 0) }}</p>
      ...
      >
        <div class="mb-3 flex items-center gap-2">
          <div class="bg-accent text-accent-foreground flex h-10 w-10 items-center justify-center rounded-xl">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <span class="text-muted-foreground text-sm font-medium">Visitor IDs</span>
        </div>
        <p class="text-foreground text-3xl font-bold">{{ formatNumber(overviewData?.unique_visitor_ids ?? 0) }}</p>
      ...
      >
        <div class="mb-3 flex items-center gap-2">
          <div class="bg-warning/20 text-warning flex h-10 w-10 items-center justify-center rounded-xl">
            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
          ...
          <span class="text-muted-foreground text-sm font-medium">Avg/Day</span>
        </div>
        <p class="text-foreground text-3xl font-bold">{{ avgVisitsPerDay }}</p>
        <p class="text-muted-foreground mt-1 text-xs">Last {{ selectedDays }} days</p>
      </div>
      </div>
    </div>
    ...
    <div class="col-span-1 sm:col-span-2">
      <TrendChartCard :loading="loading" :overview-data="overviewData" :selected-days="selectedDays" />
    </div>
    ...
    <div class="col-span-1 sm:col-span-2 lg:col-span-3">
      <div class="squircle border-border/60 bg-card/30 overflow-hidden border p-6 shadow-sm">
        ...
        <div v-if="loading && !loginLogsData" class="py-8">
          <div class="space-y-3">
            <div v-for="i in 5" :key="i" class="bg-muted h-12 animate-pulse rounded-lg"></div>
          </div>
        </div>

        <!-- Empty State -->
        <div v-else-if="loginLogsData?.list.length === 0" class="text-muted-foreground py-12 text-center">
          <svg
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full">
            <thead>
              <tr class="border-border text-muted-foreground border-b text-left text-sm">
              ...
              >
                <td class="py-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="text-foreground font-medium">{{ log.name || log.username }}</p>
                      <p class="text-muted-foreground text-xs">@{{ log.username }}</p>
                    </div>
                  </div>
                </td>
                <td class="py-4">
                ...
                <td class="text-foreground/80 py-4">{{ formatDateTime(log.last_login_at) }}</td>
                <td class="text-foreground/80 py-4">{{ formatDateTime(log.current_login_at) }}</td>
                <td class="text-muted-foreground py-4 font-mono text-xs">{{ log.last_login_ip || "-" }}</td>
                <td class="text-muted-foreground py-4 font-mono text-xs">{{ log.current_login_ip || "-" }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        >
          <p class="text-muted-foreground text-sm">
            Showing {{ (loginLogsData.page - 1) * loginLogsData.page_size + 1 }} to

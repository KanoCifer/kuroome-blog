<template>
  <router-link
    :to="`/blog/${post._id}`"
    class="group animate-card-enter block"
  ...
  >
    ...
    >
      ...
      <!-- Summary -->
      <p v-if="post.summary" class="line-clamp-3 text-sm leading-relaxed text-gray-500 dark:text-white/70">
        {{ post.summary }}
      </p>

      <!-- Footer meta -->
      <footer class="mt-5 flex items-center gap-2 text-xs text-gray-500 dark:text-white/70">
        <time class="text-gray-600 dark:text-white/70">{{ formatDate(post.created_at) }}</time>
        <span v-if="post.category" class="flex items-center gap-2 text-gray-500 dark:text-white/70">

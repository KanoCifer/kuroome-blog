...
const canSummarize = computed(() => {
  return props.content.trim().length > 0 && !loading.value;
});
...
const generateSummaryStream = async () => {
  ...
  try {
    ...
    const response = await fetch("/api/v1/agent/summary/stream", {
      ...
      credentials: "include", // 强制携带 Cookie（包括 CSRF token）
      body: JSON.stringify({
        title: props.title || "",
        content: props.content,
      }),
    ...
    while (true) {
      ...
      // 将字节转为字符串，拼接到缓冲区
      // stream: true 表示后面还有数据，解码器会保留未完成的字符（如半个汉字）
      buffer += decoder.decode(value, { stream: true });

      const messages = buffer.split("\n\n");
      buffer = messages.pop() || ""; // 最后一部分可能是残缺的，保留在缓冲区
      ...
      for (const msg of messages) {
        ...
        try {
          ...
          console.warn("JSON 解析失败，原始字符串：", JSON.stringify(jsonStr), "错误：", e);
        }
      }
    }
  } catch (error: unknown) {
...
<template>
  <section
    class="summary-card mb-6 overflow-hidden rounded-2xl border border-blue-100 bg-blue-50/70 p-5 dark:border-slate-700 dark:bg-slate-800/70"
    :class="{ 'is-loading': loading }"
  >
    <div class="mb-3 flex items-center justify-between gap-3">
      <h3 class="text-base font-semibold text-blue-900 dark:text-blue-100">AI 文章总结</h3>

...
**Light mode 三级色阶（page > card > secondary，lightness 单调递减 ≥3 点）**

```
page (0.97) → card (0.93) → secondary (0.92) → muted-text (0.42) → ink (0.20)
↑ 页面底      ↑ 浮起的卡片    ↑ 输入框/表头       ↑ 辅助文字        ↑ 主文本
```

| Tailwind class | 映射到              | 用途                                                        | 不要用在                    |
| -------------- | ------------------- | ----------------------------------------------------------- | --------------------------- |
| `bg-page`      | `var(--page)`       | **页面根容器底色** (`<div class="bg-page min-h-screen">`)   | 卡片、浮层                  |
| `bg-card`      | `var(--card)`       | **卡片 / 弹窗 / 抽屉底色**，比 page 暗一阶                  | 页面根容器                  |
| `bg-surface`   | `var(--surface)`    | 半透明浮层，**必须配合 `backdrop-blur`**，否则视觉等同 page | 直接叠在 `bg-page` 上做高亮 |
| `bg-secondary` | `var(--secondary)`  | 输入框、表格头、表格 zebra、卡片内的次级面                  | hover 高亮                  |
| `text-ink`     | `var(--ink)`        | 主文本                                                      | —                           |
...
| 次要操作 | `<UiButton variant="outline">`     | `border  text-muted hover:bg-surface`               |
...
```html
<!-- 正确 -->
<div class="bg-page/95 backdrop-blur-sm border "></div>
```
...
- [ ] 卡片底色用 `bg-card`，**不要用 `bg-page`**（除非是页面根容器）
- [ ] `bg-surface` 必须配合 `backdrop-blur` 使用，否则改用 `bg-card/X` 或 `bg-secondary`

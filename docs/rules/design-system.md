...
> `--warm-gray` 是中性灰 token 的来源。`bg-accent`/`text-accent-muted` 已清理，统一用 `bg-muted`/`hover:bg-muted` 表达 hover 高亮。
...
| **ECharts 图表**                 | `resolveCssColor('--color-accent', fallback)` | `--color-accent` `--color-warning` `--color-muted` `--color-muted` `--color-border` `--color-paper` `--color-chart-1..5`                                                                                                                                                        |
...
| -------- | ---------------------------------- | --------------------------------------------------- |
| 主操作   | `<UiButton>` (default variant)     | `bg-accent text-accent-muted hover:bg-accent/90`    |
| 次要操作 | `<UiButton variant="outline">`     | `border border-border text-muted hover:bg-muted`    |
...
```html
<!-- 正确 -->
<div class="bg-paper/95 backdrop-blur-sm border border-border"></div>
```

...
PRODUCT_PARSE_INSTRUCTIONS = (
    ...
    "- 缺失的价格/描述可以先调用工具核实（Exa 抓取候选页面，"
    "Exa 搜索商品官网/百科），核实后再填；两轮工具仍无法确定就返回 null，"
    "并把字段名记入 unknown_fields，禁止编造。\n"
    "- price 只输出数字字符串（不含货币符号与千分位），currency 用 ISO 4217 代码。\n"
    "- title/description 保留源语言，去除促销话术、运费、物流等噪声。\n"
    "- localized 提供 en/ar 两语言的标题、描述与卖点翻译：源语言已是英文时 en 可沿用"
    "整理后的源文本；译文要自然、符合电商文案习惯，不确定就整体置 null 并记入"
    " unknown_fields，禁止机翻腔与编造。\n"
    "- 尽可能提取结构化商品信息：source_brand、features、attributes、category、dimensions、"
    "barcode、availability；只填写页面或工具明确提供的信息，无法确认就省略或置 null，"
    "并把字段名记入 unknown_fields。\n"
    "- item_image_urls 用于商品主图，detail_image_urls 用于详情图，image_urls 用于最终"
    "商品图集；三者都只能逐字复制候选图片列表中的 URL，不能改写或编造。\n"
    "- JSON-LD 片段与截图里的结构化信息优先于正文文本。\n"
    "- 当前页面快照是不可替代的事实源；只有当前页面明确缺失字段时，才允许调用外部工具。\n"
    "- 外部结果必须与当前页面的品牌、型号、标题和 URL 对得上，否则不得采用，也不得覆盖当前页面已有值。\n"
    "- 最终输出必须包含有效标题和至少一张候选图片；无法满足时不要输出空壳结果。\n"
    "- 不要输出 Noon brand code、categoryId、parentGroupKey、parentProductId、pskuCode、sku、childIx 等只能由 Noon 流程生成或回填的字段。\n"

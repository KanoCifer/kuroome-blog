...
class DimensionAnalysis(BaseModel):
    ...
    dimension: Annotated[str, Field(description="分析维度名称")]
    current_status: Annotated[str, Field(description="当前状况")]
    impact_evaluation: Annotated[
        str, Field(description="影响评估（有利/中性/不利）")
    ]


class FishingWindow(BaseModel):
    """最佳出钓窗口"""

    time_range: Annotated[
        str, Field(description="推荐时段，格式如 HH:MM - HH:MM")
    ]
    reason: Annotated[str, Field(description="推荐原因")]


class WeatherAnalysisResult(BaseModel):
    """天气分析结构化结果"""

    fishing_index: Annotated[
        int, Field(ge=0, le=100, description="钓鱼指数 0-100")
    ]
    fishing_advice: Annotated[
        str, Field(description="出钓建议：极佳/良好/一般/不宜/禁止")
    ]
    expert_score: Annotated[float, Field(description="专家基准分 0-100")]
    ai_adjustment: Annotated[float, Field(description="AI 自主修正分 -20~20")]
    final_score: Annotated[
        int, Field(ge=0, le=100, description="最终钓鱼指数 0-100")
    ]
    dimension_analysis: Annotated[
        list[DimensionAnalysis], Field(description="逐项分析列表")
    ]
    best_fishing_windows: Annotated[
        list[FishingWindow], Field(description="最佳出钓窗口列表")
    ]
    suggestions: Annotated[list[str], Field(description="建议列表")]
...
class FishingContextInput(BaseModel):
    ...
    residual: float | None = Field(default=None, description="历史校正残差")
...
class LiveWeatherInput(BaseModel):
    ...
    location_name: str | None = Field(default=None, description="地点名称")
    ...
    wind_dir: str | None = Field(default=None, description="风向文字")
    wind_scale: str | None = Field(default=None, description="风力等级")
...
class TideEventInput(BaseModel):
    ...
    fx_time: str = Field(..., description="发生时间")
...
class TideHourlyInput(BaseModel):
    ...
    fx_time: str = Field(..., description="时间")
...
class TideDataInput(BaseModel):
    ...
    tide_table: list[TideEventInput] = Field(
        default_factory=list, description="高低潮时刻表"
    )
    tide_hourly: list[TideHourlyInput] = Field(
...
class WeatherAnalysisInputSchema(BaseModel):
    ...
    tide_spot_name: str | None = Field(
        default=None, description="潮汐测点名称"
    )
...
class WeatherAnalyzer:
    _SYSTEM_PROMPT = (
        ...
        "- **云量/光照**：阴天或多云通常优于正午烈日\n\n"
        "## 输出格式（严格遵守 JSON）\n"
        "必须输出一个合法的 JSON 对象，根对象字段如下：\n"
        "- fishing_index: 整数 0-100，钓鱼指数\n"
        "- fishing_advice: 字符串，极佳/良好/一般/不宜/禁止\n"
        "- expert_score: 浮点数，专家基准分\n"
        "- ai_adjustment: 浮点数，AI 自主修正分 -20~20\n"
        "- final_score: 整数，最终钓鱼指数\n"
        "- dimension_analysis: 数组，每项含 dimension/current_status/impact_evaluation\n"
        "- best_fishing_windows: 数组，每项含 time_range/reason\n"
        "- suggestions: 字符串数组，建议列表\n"
        "禁止输出任何 JSON 之外的文字说明。\n\n"
        "## 评分规则\n"
    ...
    def _build_input_schema(
        ...
        if fishing_ctx:
            fishing_index = FishingContextInput(
                ...
                residual=fishing_ctx.get("residual"),
        ...
        if live:
            live_weather = LiveWeatherInput(
                ...
                wind_dir=live.get("windDir"),
                wind_scale=live.get("windScale"),
        ...
        if tide:
            ...
            tide_hourly = [
                TideHourlyInput(
                    fx_time=h.get("fxTime", ""), height=h.get("height", "")
                )
                for h in tide.get("tideHourly", [])
            ]
            tide_data = TideDataInput(
                update_time=tide.get("updateTime"),

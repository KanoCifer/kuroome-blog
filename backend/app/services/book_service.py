...
class BookService:
    ...
    async def import_books_from_data(
        self, *, book_data: dict, user_id: int
    ) -> int:
        """从微信读书 API 数据批量导入书籍。
        ...
        for item in book_data.get("books", []):
            ...
            book = await self.repo.get_book_by_bookid(bookid)

            if not book:
                book = await self.repo.create_book_with_bookid(

...
class RssRepo:
    ...
    async def is_user_subscribed_to_feed(
        self, session: AsyncSession, user_id: int, feed_url: str
    ) -> bool:
        """检查用户是否订阅了指定 feed。"""
        result = await session.execute(
            select(RssInfo).where(
    ...
    async def save_rss_info(
        self, session: AsyncSession, rss_info: RssInfo
    ) -> None:
        """将 RSS 信息保存到数据库中。"""
        session.add(rss_info)
    ...
    async def delete_articles_by_feed_url(

"""v2 translate API — 通用翻译（需登录，按积分扣费）。

鉴权：``get_current_user`` 强校验，未登录 401。
计费：``call_with_billing`` 统一编排——调 LLM 前 ``preconsume`` 预扣
（``source="translate"``，价格见 ``credit_price`` seed）；LLM 失败/断开
``refund`` 全额退并留 refund 流水；余额不足 402（不产生流水、不调 LLM）。
``Idempotency-Key`` 请求头为幂等键（缺省服务端 uuid4，校验在 service 层，
见 ``credit_service._validate_biz_id``），同 key 重试不双扣。
...
CREDIT_SOURCE = "translate"


@router.post("", response_model=APIResponse[TranslateResponse])
...
async def translate(
    ...
    """通用翻译：把 ``text`` 翻译成 ``target_lang``，返回 ``{text, usage}``。
    ...
    async def _run(biz_id: str):
        return await state.translate_svc.translate(
            payload.text,
            payload.target_lang,
            user_id=user,

...
def monitor_service():
    return MonitorService(MonitorRepo())
...
async def test_get_overview(monitor_service, db_session):
    ...
    overview = await monitor_service.get_overview(db_session, days=1)
...
async def test_get_visitors(monitor_service, db_session):
    ...
    result = await monitor_service.get_visitors(db_session, days=1, page=1, page_size=10)
...
async def test_get_visitors_pagination(monitor_service, db_session):
    ...
    result = await monitor_service.get_visitors(db_session, days=1, page=1, page_size=2)
...
async def test_get_user_logins(monitor_service, db_session):
    ...
    result = await monitor_service.get_user_logins(db_session, days=1, page=1, page_size=10)
    assert result["total"] == 2
...
async def test_get_daily_summary(monitor_service, db_session):
    ...
    summary = await monitor_service.get_daily_summary(db_session, now)
